...
library(psych)
journals %>% 
  select(citeprice,subs) %>%      #這行多了%>%因為這樣才會存起來，並不是只顯示出來
  pairs.panels()   #打了這個為了可畫出四個圖
```
...
#pipe expression
journals %>% 
  select(citeprice, subs) -> tempDF

tempDF %>% pairs.panels()
```

```{r}
# %>% 的使用方法：journal %>% output of前面＝input of 後面 %>% 某些東西
journals %>% 
  select(citeprice,subs) %>%
  pairs.panels()

# 跑出的圖：citeprice=x;subs=y
```

```{r ols-paircorr2-r}
journals %>% 
  select(citeprice,subs) %>%
  mutate_all(log) %>%     # mutate:改變，把所有變數取log(R中內定為自然對數，你可以改)
  pairs.panels()     # pairs.panels一定要dataframe

# note：多用 ?dplyr::mutate_all 查函數的使用
```
